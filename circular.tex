...
In \citep{Lettl1990}, it is proven that the previous definition of $C(k)$ gives the same group as Sinnott's original definition. One of the reasons that $C(k)$ is important is the following famous result, due to Sinnott:
\begin{theorem}
...
\end{lemma}
\begin{proof}
It's clear that these fields are all real and abelian (their Galois groups are quotients of $G$).\dots
\end{proof}

To construct a basis of $D^+$, we can take the union of all bases for the fields $$k\cap K_1K_2K_3,k\cap K_1K_2K_4,k\cap K_1K_3K_4,k\cap K_2K_3K_4$$ (we can use the results in \citep{Kucera2016} to find these) and add in
\begin{equation*}
\begin{split}
...
&+(a_1a_2+a_1a_3+a_1a_4+a_2a_3+a_2a_4+a_3a_3)m-a_1-a_2-a_3-a_4+1.
\end{align*}
...
\item $0\le x_1 < a_1m-1, a_2 < x_2 < a_2m-1, 1\leq x_3 < n_3, x_4=0$,
\item $0\le x_1 < a_1, x_2=a_2, 1\leq x_3 < n_3-1, x_4=0$.
\end{itemize}
\paragraph*{}
...
\paragraph*{}
Next, we will recover the cases $$x_1=a_1m-1, a_2< x_2 < a_2m-1, 1\leq x_3 <n_3$$ using the relation $R_1N_1\sim 0$ and subsequently the cases $$0\leq x_1 < a_1m-1, a_2< x_2 < a_2m-1, x_3 = 0$$ and $$0\le x_1 < a_1, x_2 = a_2, x_3 = 0$$ using the relation $R_3N_3\sim 0$.
...
Thus the induction is complete and we have recovered all conjugates of $\eta$.
...
\paragraph*{}
Now consider the map $$\{0,1,\dots,\gcd(n_1,n_2)\}\times \{0,1,\dots,\gcd(n_1,n_2)\} \to \{0,1,\dots,n_1\}\times \{0,1,\dots,n_2\}$$
given by $(u,v)\mapsto (u+v\pmod{n_1},v \pmod{n_2})$. Suppose that both $(u,v)$ and $(u',v')$ map to the same element. Then $$(u-u')+(v-v')=qn_1$$ and $$v-v'=q'n_2,$$ hence $$(u-u')=qn_1-q'n_2\equiv 0 \pmod {\gcd(n_1,n_2)}.$$ Since $0\le u,u'\le \gcd(n_1,n_2)$, this implies $u=u'$. Consequently both $n_1$ and $n_2$ divide $v-v'$, hence so does $\lcm(n_1,n_2)$ and $v=v'$ (using that $0\le v,v'\le \lcm(n_1,n_2)$). Thus we have shown that the above map is injective, and since both sets have cardinality $n_1n_2$, it must be a bijection. Therefore for any $0\leq x_1<n_1$, $0\leq x_2<n_2$, we can write $$\sigma_1^{x_1}\sigma_2^{x_2}\cdot \mu=\sigma_1^{u+v}\sigma_2^{v}\cdot \mu$$
