...
\usepackage{a4wide}
\usepackage{amsmath, amsthm, amsfonts, amssymb, graphicx, url, fancyhdr,multicol,enumerate,mathtools,csquotes,tikz,enumitem,calc}
...
\begin{definition}
The \textit{genus field} (in the narrow sense) of an abelian field is its maximal abelian extension (i.e., finite Galois extension with an abelian Galois group) unramified at all (finite) primes.
\end{definition}
...
\begin{definition}
Let $G$ be any group. The (integral) \textit{group ring} $\Z[G]$ is the free $\Z$-module with basis $G$, which is made into a ring by using the group law on $G$ and extending linearly.
\end{definition}

\begin{definition}
An element $\alpha$ of a number field $K$ is called totally positive if for any embedding $\sigma: K\to \R$, we have $\sigma(\alpha)>0$.
\end{definition}

\section{The group of circular numbers}
Let $k$ a real abelian field, $K$ its the genus field in the narrow sense, $P$ is the set of ramified primes of $k$, $K_p$ is the maximal subfield of $K$ ramified only at $p\in P$. Since $\Gal(K/k)$ has a natural action on $K$ (given by evaluating an automorphism on an element), this makes $K$ into a $\Z[\Gal(K/k)]$-module. 

\begin{definition}
The group $D(k)$ of circular numbers of $k$ (using Lettl's modification of Sinnott's definition) is given as
$$D:=\big\langle \{-1, \eta_I \big\vert \emptyset \subsetneq I \subseteq P \}\big\rangle_{\Z[\Gal(K/k)]},$$
where $\langle \dots \rangle_{\Z[\Gal(K/k)]}$ means \enquote{generated as a $\Z[\Gal(K/k)]$-submodule of $K$} and $$\eta_I=\text{N}_{\Qbb(\zeta_{\text{cond} \left(\prod_{i\in I}K'_i\right)})/\left(\prod_{i\in I}K'_i)\right)\cap k'}\left(1-\zeta_{\text{cond} \left(\prod_{i\in I}K'_i\right)}\right),$$ where $\text{N}$ denotes the norm operator. The subset of totally positive elements of $D(k)$ will be denoted by $D^+(k)$.
\end{definition}
...
\item For $I=\{p\}$, we have $\eta_I\not\in E(k)$, but $\eta_I^{1-\sigma}\in E(k)$ for any $\sigma$ in the inertia subgroup of $\Gal(K/k)$ corresponding to $p$.
\end{enumerate}
\end{lemma}
\begin{proof}
\end{proof}

The next result shows that $D^+$ and $C^+$ are free $\Z$-modules.
\begin{lemma}
$D^+(k)$ is a subgroup of $D(k)$ given as $$D(k)=\big\langle -1, \eta_I \big\vert \emptyset \subsetneq I \subseteq P\big\rangle_{\Z[\Gal(K/k)]},$$
...
\begin{lemma}
The $\Z$-rank of $D^+$ is $[k:\Q]+|P|-1$.
\end{lemma}
\begin{proof}
\end{proof}

\section{Notation and assumptions}
In the remainder of the thesis, we will fix $k$ to be a real abelian field with exactly four ramified primes $p_1,p_2,p_3,p_4$. Let $K$ be the genus field in the narrow sense of $k$. Let $G:=\Gal(K/\Q)$, then by lemma \ref{genus}, we can identify $G$ with the direct product $T_1\times T_2\times T_3\times T_4$, where $T_i$ is the inertia group corresponding the ramified prime $p_i$. Next, we will define:
...
\paragraph*{}
Let $Q'$ be the quotient $\Z[G]$-module $$D^+/\big\langle \{-1, \eta_I \big\vert \emptyset \subsetneq I \subsetneq P \}\big\rangle_{\Z[\Gal(K/k)]}$$
and let $Q$ be the quotient $\Z$-module of $Q'$ by the conjugates we can already recover, i.e
\begin{align*}
Q:=Q'/\big\langle \{\eta^{\sigma_1^{x_1}\sigma_2^{x_2}\sigma_3^{x_3}\sigma_4^{x_4}}; \quad & 0\leq x_1< n_1, 0\leq x_2<n_2, 1< x_3<n_3,0\leq x_4<a_4,\\
\text { or }& 0\leq x_1< n_1-r_2-1, 0\leq x_2<n_2, x_3=x_4=0,\\
\text { or }& x_1= n_1-r_2-1, 0\leq x_2<r_1+r_3-2, x_3=x_4=0 \}\big\rangle_{\Z}.
\end{align*}
We will write $Q$ additively and for any $\rho\in\Gal(k/\Q)$, we will denote the image of $\eta^{\rho}$ in $Q$ by $\rho\cdot \mu$.
...
\begin{lemma}
For any $0\leq x_1\leq tu-1, 0\leq x_2\leq tv-1$, we have
$$
...
X_{-x_1+x_2+1} \quad &\!\begin{aligned}\text{ if } & t(u-1)-1< x_1<tu-1 \\&
...
\paragraph*{}
Let $\alpha:= Y_{v+t-2}+\dots+Y_{tv-1}$ and $\beta:=X_1+X_2+\dots+X_t$.
\begin{lemma}
We have $\alpha=\beta=0$.
\end{lemma}
\begin{proof}
Using the relation $N_2\sim 0$, we have$$0=\sigma_1^{n_1-r_1-1}N_2\cdot \mu =\sum_{x_2=0}^{tv-1}  \sigma_1^{t(u-1)}\sigma_2^{x_2}\cdot \mu= \alpha$$ and
...
Similarly, using the relation $N_1\sim 0$, we have $$0= N_1\cdot \mu=\dots=u\cdot \beta.$$ Since $\gcd(u,v)=1$, this implies $\beta=0$ by Bezout's identity.
\end{proof}
...
Since the rows of $M$ are the coefficients of valid equlities in $Q$, we have $M\cdot X'=0$, where $X=(X_1,\dots,X_t)$ and $'$ denotes transposition. We will show that $M$ is unimodular, i.e. invertible over $\Z$, from which it will follow that $X=0$. To do that, we will first need to describe $M$ in a better way.
